...
Run exactly one review/fix pass for the current repository unless the user explicitly asks to start continuous mode. This skill is a workflow harness for Codex behavior, not an in-session infinite runner. A clean pass is valid when no real P0/P1 issue exists.

## State File

Use `.codex/review-loop.md` in the repository root as the loop log. Create it if missing.

Before reviewing:

- Read `.codex/review-loop.md`.
...
After the pass, update `.codex/review-loop.md` with:
...
1. Current user instructions for this run.
2. Repository file `.codex/review-loop.prompts.md`.
3. Defaults from `references/default-prompts.md`.
...
Record the prompt source used for each phase in `.codex/review-loop.md`, for example `default`, `user instruction`, or `.codex/review-loop.prompts.md#Review Prompt`.
...
- Record the branch and PR URL or number in `.codex/review-loop.md`.
...
9. Update `.codex/review-loop.md`.
10. Stop after this single pass.

## Continuous Mode

Use this section only when the user explicitly asks to keep looping, create new sessions, run repeatedly, continue until clean, or start an automation.

Do not implement continuous mode as an infinite loop inside the current session. Use a Codex automation or an external runner so each run is a fresh standalone pass.
